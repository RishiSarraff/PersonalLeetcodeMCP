// src/helpers
import type { Outcome, Difficulty, TagProblemCounts, SubmissionEntry } from "./types.js";

// ─── Spaced repetition ───────────────────────────────────────────────────────

export const BOX_INTERVALS_DAYS: number[] = [1, 2, 4, 7, 14, 30];
export function computeNextBox(currentBox: number | null, outcome: Outcome): number{
    switch(outcome){
      case "FAILED":
        return 0
      case "STRUGGLED":
...
// ─── Mock interview ──────────────────────────────────────────────────────────
const TIME_LIMITS: Record<Difficulty, number> = { EASY: 20, MEDIUM: 30, HARD: 45 };
...
export function analyzeWeakAreas(
...
): { weak: WeakStrongTag[]; strong: WeakStrongTag[]; total: number } {
    ...
    const weak = sorted.slice(0, 8).map((t) => ({ tag: t.tagName, solved: t.problemsSolved }));
    const strong = sorted.slice(-5).reverse().map((t) => ({ tag: t.tagName, solved: t.problemsSolved }));
...
export function categorizeSubmissions(submissions: SubmissionEntry[]): {
    ...
    const byProblem: Record<string, { title: string; attempts: number; accepted: boolean }> = {};
  
    for (const s of submissions) {
      byStatus[s.statusDisplay] = (byStatus[s.statusDisplay] || 0) + 1;
      byLang[s.lang] = (byLang[s.lang] || 0) + 1;
      byProblem[s.titleSlug] = byProblem[s.titleSlug] || { title: s.title, attempts: 0, accepted: false };
